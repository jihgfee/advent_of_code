...
let result1 = sum (List.map (fun x -> if check_contains x then 1 else 0) xs) in
let result2 = sum (List.map (fun x -> if check_overlaps x then 1 else 0) xs) in
Printf.printf "Contains: %i\n" result1;
Printf.printf "Overlaps: %i\n" result2
