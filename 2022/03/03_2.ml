...
(* let sorted_xs = List.map (List.sort (fun c1 c2 -> Int.compare (eval_char c1) (eval_char c2))) xs in *)
List.iter (Printf.printf "%c") (List.map eval_int (List.hd xs''));
Printf.printf "\n";
let dups = List.map (find_duplicate 3) xs'' in
List.iter (Printf.printf "%c") (List.map eval_int dups);
Printf.printf "\n";
