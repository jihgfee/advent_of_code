...
let compute_position (x1,y1) (x2,y2) =
  if x1 = x2 then (x2,y1) else 
  if y1 = y2 then (x1,y2) else
  if x1 < x2 then [O
in
...
let rec parse_instructions visited ks ds =
  ...
  | d::ds -> let (visited', ks') = parse_instruction visited ks d in
             Printf.printf "[";
             List.iter (fun (x,y) -> Printf.printf "(%i,%i)," x y) ks';
             Printf.printf "]\n";
             (* Printf.printf "(%i,%i)\n" tx' ty'; *)
             Printf.printf "----\n";
...
(* Printf.printf "["; *)
(* List.iter (fun (x,y) -> Printf.printf "(%i,%i)," x y) (PairSet.elements visited); *)
(* Printf.printf "]\n"; *)
