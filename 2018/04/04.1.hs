...
findMaxElf ((Elf id is):es) (Just (Elf id' is')) =
  findMaxElf es (if (sumIntervals is > sumIntervals is') then Just (Elf id is) else Just (Elf id' is'))
...
main = do
  ...
  let overlapMin = highestOverlap maxElf
  -- print elves
